    ...
    override fun map(context: Observation.Context): Observation.Context {
        if (context is ChatModelObservationContext) {
            ...
            if (response != null) {
                ...
                val inputTokenPerItem: Double = if (dataCount > 0) (totalInputTokens - instructionTokens).toDouble() / dataCount else 0.0
                val outputTokensPerItem = if (dataCount > 0) (totalOutputTokens - outputFormattingTokens).toDouble() / dataCount else 0.0
                ...
                context.addLowCardinalityKeyValue(
                    KeyValue.of(
                        "app.ai.usage.input_tokens",
                        totalInputTokens.toString()
                    )
                )
                context.addLowCardinalityKeyValue(
                    KeyValue.of(
                        "app.ai.usage.output_tokens",
                        totalOutputTokens.toString()
                    )
                )
                context.addLowCardinalityKeyValue(
                    KeyValue.of(
                        "app.ai.usage.total_tokens",
                ...
                context.addLowCardinalityKeyValue(
                    KeyValue.of(
                        "app.ai.usage.instruction_tokens",
                        instructionTokens.toString()
                    )
                )
                context.addLowCardinalityKeyValue(
                    KeyValue.of(
                        "app.ai.usage.input_tokens_per_item",
                        inputTokenPerItem.toString()
                    )
                )
                context.addLowCardinalityKeyValue(
                    KeyValue.of(
                        "app.ai.usage.output_tokens_per_item",
