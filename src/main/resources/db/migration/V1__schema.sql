create table app_user (id uuid not null, name varchar(255), user_role varchar(255) check (user_role in ('ADMIN','USER')), primary key (id));
create table email_source (is_active boolean not null, id uuid not null, service_provider_id uuid not null, subject_discriminator varchar(255), target_address varchar(255) not null, analyzed_message_ids jsonb, event_rules jsonb, primary key (id));
create table google_account (analyzed_at timestamp(6) with time zone, expires_at timestamp(6) with time zone, app_user_id uuid not null, access_token TEXT, email varchar(255), name varchar(255), refresh_token TEXT, scope varchar(255), subject varchar(255) not null, primary key (subject));
