...
class EmailSource(
    ...
    fun hasPaymentStartRule() =
        eventRules.any { it.eventType == SubscriptionEventType.SUBSCRIPTION_START_OR_PAYMENT && it.isActive && !it.isMonthlyRecurring }

    fun hasPaymentCancelRule() =
        eventRules.any { it.eventType == SubscriptionEventType.SUBSCRIPTION_CANCEL && it.isActive }

    fun hasMonthlyPaymentRule() =
        eventRules.any { it.eventType == SubscriptionEventType.SUBSCRIPTION_START_OR_PAYMENT && it.isActive && it.isMonthlyRecurring }
